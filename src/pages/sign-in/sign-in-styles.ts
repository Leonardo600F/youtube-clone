import styled from "styled-components";

interface InputValid { valid: boolean }

interface IsFocused { isFocused: boolean }

interface PasswordFocused { passwordFocused: boolean }

export const MainContainer = styled.div`
display: flex;
height: 100vh;
width: 100vw;
background-color: #d2d2d2`;

export const LoginContainer = styled.div`
position: absolute;
width: 1075px;
height: 470px;
top: 50%;
left: 50%;
transform: translate(-50%, -50%);
border-radius: 30px;
background-color: #fff;

@media(max-width: 1760px) {width: 925px}

@media(max-width: 1200px) {
width: 785px;
height: 440px}

@media(max-width: 830px) {
width: 710px;
height: 430px}

@media(max-width: 730px) {
width: 620px;
height: 440px}

@media(max-width: 650px) {
width: 570px;
height: 420px}

@media(max-width: 599px) {
height: 100vh;
width: 100vw;
border-radius: 0px}`;

export const GoogleIconContainer = styled.img`
height: 50px;
width: 50px;
margin: 40px 0 0 40px;
border-radius: 50%;

@media(max-width: 1200px) {
width: 45px;
height: 45px;
margin-top: 35px}

@media(max-width: 730px) {
height: 43px;
width: 43px}

@media(max-width: 599px) {
height: 53px;
width: 53px;
margin: 20px 0 0 15px}`;

export const LoginSpanContainer = styled.div`
span {
display: flex;
width: fit-content;
cursor: default}

span:first-child {
font-size: 45px;
margin: 9px 0 0 40px;

@media(max-width: 730px) {font-size: 38px}

@media(max-width: 599px) {
margin: 4px 0 0 22px;
font-size: 30px}}

span:last-child {
font-size: 16px;
margin: 9px 0 0 40px;

@media(max-width: 730px) {font-size: 15px}

@media(max-width: 599px) {margin: 4px 0 0 22px}}`;

export const EmailInfoContainer = styled.div<InputValid>`
display: flex;
height: ${({ valid }) => valid ? '180px' : '205px'};
width: 537px;
margin: -195px 0 0 537px;
border-top-right-radius: 30px;

@media(max-width: 1760px) {margin-left: 387px}

@media(max-width: 1680px) {
height: ${({ valid }) => valid ? '175px' : '205px'};
width: 490px;
margin: -192px 0 0 435px}

@media(max-width: 1200px) {
height: ${({ valid }) => valid ? '160px' : '185px'};
width: 423px;
margin: -182px 0 0 361px}

@media(max-width: 830px) {
width: 393px;
margin-left: 317px}

@media(max-width: 730px) {
height: ${({ valid }) => valid ? '155px' : '182px'};
width: 360px;
margin: -171px 0 0 260px;}

@media(max-width: 650px) {
height: ${({ valid }) => valid ? '155px' : '180px'};
width: 310px;
margin-top: -170px}

@media(max-width: 599px) {
width: 100%;
height: ${({ valid }) => valid ? '82px' : '110px'};
border-radius: 0px;
margin: 0px}`;

export const EmailContainer = styled.div<IsFocused>`
display: flex;
height: 55px;
width: 470px;
margin: 115px 0 0 10px;
border: solid;
border-radius: 5px;
border-width: ${({ isFocused }) => isFocused ? '2px' : '1px'};
border-color: ${({ isFocused }) => isFocused ? '#4169E1' : 'grey'};

@media(max-width: 1680px) {width: 440px}

@media(max-width: 1200px) {
width: 370px;
margin-top: 97px}

@media(max-width: 830px) {width: 345px}

@media(max-width: 730px) {
width: 320px;
margin: 92px 0 0 0}

@media(max-width: 650px) {width: 265px}

@media(max-width: 599px) {
height: 55px;
width: 510px;
margin: 20px 0 0 20px}

@media(max-width: 535px) {width: 445px}

@media(max-width: 480px) {width: 415px}

@media(max-width: 450px) {width: 385px}

@media(max-width: 413px) {width: 330px}

@media(max-width: 360px) {width: 300px}

@media(max-width: 330px) {width: 280px}`;

export const EmailInput = styled.input<InputValid>`
width: 100%;
height: 100%;
max-width: ${({ valid }) => valid ? '458px' : '462px'};
max-height: 53px;
margin: ${({ valid }) => valid ? 'none' : '-2px 0 0 -2px'};
border: ${({ valid }) => valid ? 'none' : 'solid 3px'};
border-color: red;
border-radius: 5px;
padding-left: 10px;
font-size: 16px;
outline: none;
background-color: #fff;

@media(max-width: 1680px) {max-width: 427px}

@media(max-width: 1200px) {max-width: 358px}

@media(max-width: 830px) {
max-width: 333px;
max-height: 53px}

@media(max-width: 730px) {max-width: 307.5px}

@media(max-width: 650px) {max-width: 252px}

@media(max-width: 599px) {max-width: 498px}

@media(max-width: 535px) {max-width: 433px}

@media(max-width: 480px) {max-width: 403px}

@media(max-width: 450px) {max-width: 373px}

@media(max-width: 413px) {max-width: 318px}

@media(max-width: 360px) {max-width: 288px}

@media(max-width: 330px) {max-width: 268px}

&:focus + label,
&:not(:placeholder-shown) + label {
height: 15px;
width: 115px;
font-size: 12px;
transform: translateY(-27px);
color: ${({ valid }) => valid ? '#4169E1' : 'red'};

@media(max-width: 730px) {transform: translateY(-28px)}

@media(max-width: 500px) {transform: translateY(-30px)}`;

export const EmailLabel = styled.label<InputValid>`
height: 21px;
width: 130px;
margin: ${({ valid }) => valid ? '16px 0px 0px -455px' : '18px 0 0 -462px'};
font-size: 16px;
transition: 0.2s ease all;
pointer-events: none;
color: ${({ valid }) => valid ? '#333' : 'red'};
background-color: #fff;

@media(max-width: 1680px) {margin: ${({ valid }) => valid ? '18px 0px 0px -427px' : '20px 0 0 -430px'}}

@media(max-width: 1200px) {margin: ${({ valid }) => valid ? '20px 0px 0px -357px' : '21px 0 0 -364px'}}

@media(max-width: 830px) {margin: 19px 0px 0px -330px}

@media(max-width: 730px) {margin-left: -308px}

@media(max-width: 650px) {margin-left: ${({ valid }) => valid ? '-250px' : '-255px'}}

@media(max-width: 599px) {margin-left: -500px}

@media(max-width: 535px) {margin-left: -430px}

@media(max-width: 480px) {margin-left: -405px}

@media(max-width: 450px) {margin-left: -375px}

@media(max-width: 413px) {margin-left: -320px}

@media(max-width: 360px) {margin-left: -290px}

@media(max-width: 330px) {margin-left: -270px}`;

export const ExclamationIconContainer = styled.img`
width: 20px;
height: 20px;
border-radius: 50px`;

export const PasswordInfoContainer = styled.div<InputValid>`
display: flex;
width: 537px;
height: ${({ valid }) => valid ? '73px' : '98px'};
margin-left: 537px;

@media(max-width: 1760px) {margin-left: 387px}

@media(max-width: 1680px) {
width: 490px;
margin-left: 435px}

@media(max-width: 1200px) {
width: 423px;
margin-left: 361px}

@media(max-width: 830px) {
width: 393px;
margin-left: 317px}

@media(max-width: 730px) {
width: 360px;
margin-left: 260px}

@media(max-width: 650px) {width: 310px;}

@media(max-width: 599px) {
width: 100%;
height: ${({ valid }) => valid ? '72px' : '98px'};
margin: 0}`;

export const PasswordContainer = styled.div<PasswordFocused>`
display: flex;
height: 55px;
width: 470px;
margin: 10px 0 0 10px;
border: solid;
border-radius: 5px;
border-width: ${({ passwordFocused }) => passwordFocused ? '2px' : '1px'};
border-color: ${({ passwordFocused }) => passwordFocused ? '#4169E1' : 'grey'};

@media(max-width: 1680px) {width: 440px}

@media(max-width: 1200px) {width: 370px}

@media(max-width: 830px) {width: 345px}

@media(max-width: 730px) {
width: 320px;
margin-left: 0}

@media(max-width: 650px) {width: 265px}

@media(max-width: 599px) {
height: 55px;
width: 510px;
margin: 10px 0 0 20px}

@media(max-width: 535px) {width: 445px}

@media(max-width: 480px) {width: 415px}

@media(max-width: 450px) {width: 385px}

@media(max-width: 413px) {width: 330px}

@media(max-width: 360px) {width: 300px}

@media(max-width: 330px) {width: 280px}`;

export const PasswordInput = styled.input<InputValid>`
width: 100%;
height: 100%;
max-width: ${({ valid }) => valid ? '458px' : '462px'};
max-height: 53px;
margin: ${({ valid }) => valid ? 'none' : '-2px 0px 0 -2px'};
border: ${({ valid }) => valid ? 'none' : 'solid 3px'};
border-color: red;
border-radius: 5px;
padding-left: 10px;
font-size: 16px;
outline: none;
background-color: #fff;

@media(max-width: 1680px) {max-width: 427px}

@media(max-width: 1200px) {max-width: 358px}

@media(max-width: 830px) {
max-width: 333px;
max-height: 53px}

@media(max-width: 730px) {max-width: 307.5px}

@media(max-width: 650px) {max-width: 252px}

@media(max-width: 599px) {max-width: 498px}

@media(max-width: 535px) {max-width: 433px}

@media(max-width: 480px) {max-width: 403px}

@media(max-width: 450px) {max-width: 373px}

@media(max-width: 413px) {max-width: 318px}

@media(max-width: 360px) {max-width: 288px}

@media(max-width: 330px) {max-width: 268px}
&:focus + label,
&:not(:placeholder-shown) + label {
height: 15px;
width: 102px;
padding-left: 0px;
font-size: 12px;
transform: translateY(-27px);
color: ${({ valid }) => valid ? '#4169E1' : 'red'};

@media(max-width: 730px) {transform: translateY(-28px)}

@media(max-width: 500px) {transform: translateY(-30px)}`;

export const PasswordLabel = styled.label<InputValid>`
height: 21px;
width: 140px;
margin: ${({ valid }) => valid ? '16px 0px 0px -455px' : '18px 0 0 -462px'};
font-size: 16px;
transition: 0.2s ease all;
pointer-events: none;
color: ${({ valid }) => valid ? '#333' : 'red'};
background-color: #fff;

@media(max-width: 1680px) {margin: ${({ valid }) => valid ? '18px 0px 0px -427px' : '20px 0 0 -430px'}}

@media(max-width: 1200px) {margin: ${({ valid }) => valid ? '20px 0px 0px -357px' : '21px 0 0 -364px'}}

@media(max-width: 830px) {margin: 19px 0px 0px -330px}

@media(max-width: 730px) {margin-left: -308px}

@media(max-width: 650px) {margin-left: ${({ valid }) => valid ? '-250px' : '-255px'}}

@media(max-width: 599px) {margin-left: -500px}

@media(max-width: 535px) {margin-left: -430px}

@media(max-width: 480px) {margin-left: -405px}

@media(max-width: 450px) {margin-left: -375px}

@media(max-width: 413px) {margin-left: -320px}

@media(max-width: 360px) {margin-left: -290px}

@media(max-width: 330px) {margin-left: -270px}`;

export const ShowPasswordContainer = styled.div`
display: flex;
width: fit-content;
height: fit-content;
margin: 4px 0 0 548px;

span {
display: flex;
width: fit-content;
height: fit-content;
margin: 2px 0 0 10px;
font-size: 14px;
font-weight: 600;
cursor: pointer}

@media(max-width: 1760px) {margin-left: 400px}

@media(max-width: 1680px) {margin-left: 445px}

@media(max-width: 1200px) {margin-left: 372px}

@media(max-width: 830px) {margin-left: 330px}

@media(max-width: 730px) {margin-left: 265px}

@media(max-width: 650px) {margin-left: 265px}

@media(max-width: 599px) {margin-left: 20px}`;

export const StyledCheckbox = styled.input.attrs({ type: 'checkbox' })`
display: flex;
width: 18px;
height: 18px;
background-color: #7CB9E8; 
cursor: pointer`;

export const ButtonsContainer = styled.div`
display: flex;
height: 67px;
width: fit-content;
margin: 50px 0 0 520px;

@media(max-width: 1760px) {margin-left: 430px}

@media(max-width: 1680px) {margin-left: 450px}

@media(max-width: 1200px) {margin: 40px 0 0 320px}

@media(max-width: 830px) {margin: 50px 0 0 250px}

@media(max-width: 730px) {margin-left: 170px}

@media(max-width: 650px) {margin-left: 100px}

@media(max-width: 599px) {
margin-top: 37px;
margin-left: 112px}

@media(max-width: 535px) {margin-left: 47px}

@media(max-width: 480px) {margin-left: 15px}

@media(max-width: 450px) {margin-left: -18px}

@media(max-width: 413px) {margin-left: -68px}

@media(max-width: 370px) {margin-left: -75px}

@media(max-width: 360px) {margin-left: -105px}

@media(max-width: 330px) {margin-left: -125px}`;

export const SignUpContainer = styled.div`
display: flex;
height: 40px;
width: 105px;
margin: 15px 0 0 260px;
border-radius: 50px;
cursor: pointer;
background-color: #fff;

span {
width: fit-content;
height: fit-content;
margin: 11px 0 0 18px;
border-radius: 50px;
font-weight: 500;
font-size: 14px;
color: #4169E1}

&:hover {background-color: #F8F8FF}

@media(max-width: 1760px) {margin-left: 194px}

@media(max-width: 1200px) {margin-left: 180px}`;

export const NextButtonContainer = styled.div`
display: flex;
height: 40px;
width: 105px;
margin: 15px 0 0 25px;
border-radius: 50px;
cursor: pointer;
background-color:rgb(150, 193, 233);

span {
display: flex;
justify-content: center;
align-items: center;
width: 100%;
height: 100%;
border-radius: 50px;
font-weight: 500;
font-size: 14px;
color:rgb(52, 93, 214)}

&:hover {background-color: #6495ED}`;

export const EmailEmptyContainer = styled.div<InputValid>`
display: ${({ valid }) => valid ? 'none' : 'flex'};
height: 21px;
margin: 180px 0 0 -495px;
background-color: #fff;

span {
display: block;
height: 21px;
font-size: 13px;
margin-left: 5px;
color: red;
cursor: default}

@media(max-width: 1760px) {margin: 179px 0 0 -495px}

@media(max-width: 1680px) {margin-left: -462px}

@media(max-width: 1200px) {margin: 160px 0 0 -390px}

@media(max-width: 830px) {margin-left: -365px}

@media(max-width: 730px) {margin: 157px 0 0 -342px}

@media(max-width: 650px) {margin-left: -288px}

@media(max-width: 599px) {
margin: 83px 0 0 -530px;
span {font-size: 14px}}

@media(max-width: 535px) {margin-left: -465px}

@media(max-width: 480px) {margin-left: -435px}

@media(max-width: 450px) {margin-left: -405px}

@media(max-width: 413px) {margin-left: -350px}

@media(max-width: 360px) {margin-left: -320px}

@media(max-width: 330px) {
margin-left: -300px;
span {font-size: 13px}}`;

export const PasswordEmptyContainer = styled.div<InputValid>`
display: ${({ valid }) => valid ? 'none' : 'flex'};
height: 21px;
margin: 73px 0 0 -495px;
background-color: #fff;

span {
display: block;
height: 21px;
font-size: 13px;
margin-left: 5px;
color: red;
cursor: default}

@media(max-width: 1760px) {margin: 72px 0 0 -495px}

@media(max-width: 1680px) {margin: 73px 0 0 -462px}

@media(max-width: 1200px) {margin-left: -390px}

@media(max-width: 830px) {margin: 75px 0 0 -365px}

@media(max-width: 730px) {margin-left: -342px}

@media(max-width: 650px) {margin: 74px 0 0 -288px}

@media(max-width: 599px) {
margin: 73px 0 0 -530px;
span {font-size: 14px}}

@media(max-width: 535px) {margin-left: -465px}

@media(max-width: 480px) {margin-left: -435px}

@media(max-width: 450px) {margin-left: -405px}

@media(max-width: 413px) {margin-left: -350px}

@media(max-width: 360px) {margin-left: -320px}

@media(max-width: 330px) {
margin-left: -300px;
span {font-size: 13px}}`;