...
export default function PostsHome() {
    ...
    const displayPosts = useMemo(() => {
        ...
        if (windowWidth <= 2100) return posts.slice(0, 5);
        return posts;
    }, [windowWidth, posts]);
    ...
    const displaySecondPosts = useMemo(() => {
        ...
        if (windowWidth <= 2100) return secondPosts.slice(0, 5);
        return secondPosts;
        ...
        <>
            ...
            <PostsContainer openPosts={openPosts}>
                {displayPosts.map((posts) => (
                    <PostsComponents key={`post-${posts.name}`} post={posts} />
                ))}
            </PostsContainer>

            {openPosts && (
                <PostsContainer openPosts={openPosts}>
                    {displaySecondPosts.map((secondPosts, index) => (
                        <SecondPostsComponents key={`second-${secondPosts.name}-${index}`} secondPosts={secondPosts} />
                    ))}
                </PostsContainer>
            )}
